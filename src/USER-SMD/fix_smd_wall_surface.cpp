...
------------------------------------------------------------------------- */
/*这个修复程序从.STL格式的文件中读取一个三角形表面。对于每个三角形，创建一个新的粒子，该粒子存储三角形的重心和顶点位置。新粒子的半径是包围三角形顶点的最小圆的半径。

这个三角形表面可以通过smd/tri_surface配对样式作为复杂的刚性壁面使用。可以通过smd/move_tri_surf修复样式移动三角形表面。

在读取了.STL文件之后，需要运行0个时间步骤的模拟，以便正确注册系统中的新粒子。请参阅MACHDYN示例目录中的“funnel_flow”示例。*/
#include <math.h>
