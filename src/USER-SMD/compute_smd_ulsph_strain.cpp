...
/* ----------------------------------------------------------------------
 ...
 ------------------------------------------------------------------------- */
/*定义一个计算，用于输出通过更新的Lagrangian SPH对样式进行相互作用的粒子的对数应变张量。在SPH模拟中，对数应变张量用于描述物体的变形情况，并且通常表示为对变形梯度张量的自然对数。

对数应变张量（(E)）可以通过以下方式计算：

首先，计算变形梯度张量（(F)）。
然后，对变形梯度张量取自然对数，即 (E = \ln(F))。
变形梯度张量（(F)）通常是由粒子之间的相互作用和形变而导致的。在SPH模拟中，可以通过跟踪粒子的位置和速度，并应用相应的变形梯度计算方法来计算 (F)。然后，对得到的变形梯度张量取自然对数，即可得到对数应变张量 (E)。*/
#include <string.h>
