...
/* ----------------------------------------------------------------------
 ...
 ------------------------------------------------------------------------- */
/*这个命令将组中每个粒子的速度分量设置为指定的值vx、vy、vz，而不考虑作用在粒子上的力。这个命令可以用来施加速度边界条件。

可以将vx、vy、vz中的任何一个值指定为NULL，这意味着不改变该维度上的速度分量。

这个修复程序应该与一个时间积分修复程序一起使用。

vx、vy、vz中的任何一个值都可以被指定为等式样式或原子样式变量，即vx、vy、vz。如果值是一个变量，则应该将其指定为v_name，其中name是变量的名称。在这种情况下，每个时间步将计算变量的值，并使用它来确定力分量。

等式样式变量可以指定带有各种数学函数的公式，并包括用于模拟盒参数、时间步长和经过的时间的thermo_style命令关键字。因此，很容易指定一个时间相关的速度场。

原子样式变量可以指定与等式样式变量相同的公式，但也可以包括每个原子的值，比如原子坐标。因此，很容易指定一个具有空间依赖性的速度场，也可以选择包括时间依赖性。

如果使用了region关键字，则粒子必须同时位于指定的几何区域中，才能由该命令设置其速度。*/
#include <string.h>
#include <stdlib.h>
