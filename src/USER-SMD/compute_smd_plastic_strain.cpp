...
void ComputeSMDPlasticStrain::init() //计算塑性应变
{
...
void ComputeSMDPlasticStrain::compute_peratom()
{
  ...
  if (atom->nmax > nmax) {
    ...
    vector_atom = plastic_strain_vector;//用于存储每个原子的塑性应变
  }
  ...
  int nlocal = atom->nlocal;
    ...
    for (int i = 0; i < nlocal; i++) {//将需要计算的原子的塑性应变存储到plastic_strain_vector,其余不需要计算的原子的塑性应变置为0
      if (mask[i] & groupbit) {
